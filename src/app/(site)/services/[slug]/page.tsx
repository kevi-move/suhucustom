...
import ServiceFallbackActions from "@/components/services/ServiceFallbackActions";
...
async function withServiceCms(slug: string, children: ReactNode) {
  const pageSlug = `/services/${slug}`;
  const [{ content }, modeEnabled] = await Promise.all([
    getPageContent(pageSlug),
    getCmsModeEnabled(),
  ]);

  return (
    <ServicePageClient pageSlug={pageSlug} initialContent={content} modeEnabled={modeEnabled}>
      {children}
    </ServicePageClient>
  );
}
      ...
      <div className="grid gap-10 lg:grid-cols-2 lg:items-start">
        ...
        <div>
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            <ServiceFallbackActions productCategory={item.nameEn} />
